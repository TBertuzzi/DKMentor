...
local frostMarkers = {}
for _, marker in ipairs(frostMplus.treeKeyTalents or {}) do frostMarkers[marker.spellID] = true end
assert(frostMarkers[435005], "Frost M+ Smothering Offense marker missing")
assert(not frostMarkers[1229310], "Frostbane must not return as a Frost M+ guide marker")
assert(frostMplus.note:find("Frostscythe%-at%-2%+", 1, false), "Frost M+ 2+ Frostscythe threshold note missing")
assert(frostMplus.note:find("Glacial%-Advance%-at%-3%+", 1, false), "Frost M+ 3+ Glacial Advance threshold note missing")
assert(frostMplus.freshness == "current" or frostMplus.freshness == "review", "Frost M+ guidance freshness state invalid")
assert(frostMplus.sourceUpdated == "2026-09-05", "Frost M+ source date should reflect the September 5 guide refresh")
assert(DKM.Builds[252].mythicplus[1].freshness == "current" or DKM.Builds[252].mythicplus[1].freshness == "review", "Unholy M+ guidance freshness state invalid")
...
assert((DKM.AdvisorData.reviewed or "") >= "2026-09-06", "Advisor review date must not regress")
assert(DKM.AdvisorData.freshness.builds[251] == "current" or DKM.AdvisorData.freshness.builds[251] == "review", "Frost build freshness state invalid")
assert(DKM.AdvisorData.freshness.builds[252] == "current" or DKM.AdvisorData.freshness.builds[252] == "review", "Unholy build freshness state invalid")
assert((DKM.GearData.reviewed or "") >= "2026-09-08", "Gear review date must not regress")
